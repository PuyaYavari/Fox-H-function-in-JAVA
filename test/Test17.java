...
public class Test17 {
	...
	public static void main(String[] args) {
		...
		double alfa = 1;
		double arrb[] = { alfa - 0.25, alfa + 0.25, alfa - 0.5, alfa, alfa };

		a = Nd4j.ones(1).mul(alfa);
		A = Nd4j.ones(1);
		b = Nd4j.create(arrb).transpose();
		B = Nd4j.ones(5).transpose();

		double x = 11.9553345;

		System.out.println("Expected result = " + (Math.pow(x, alfa - 0.5) / (Math.sqrt(2) * Math.pow(Math.PI, 1.5)))
				* Math.cosh(2 * Math.sqrt(2) * Math.sqrt(Math.sqrt(x)))
				* Math.sin(2 * Math.sqrt(2) * Math.sqrt(Math.sqrt(x))));
